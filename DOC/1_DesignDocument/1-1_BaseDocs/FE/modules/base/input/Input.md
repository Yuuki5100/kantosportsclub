...
- **TextBox**: 単行のテキスト入力フィールド。カスタムスタイルや単位表示、クリアボタンの表示に対応。IE対応でreveal/clearボタンを非表示。
  - **主な引数**:
    - `name`: 入力フィールドの名前（必須）
    - `type`: 入力タイプ（text, number, password など）
    - `defaultValue`: 初期値
    - `onBlur`: フォーカスアウト時のイベントハンドラ
  ...
  - **機能**: 入力文字の最大長制限、補助テキスト表示、無効化スタイル、単位表示、スタイルカスタマイズ、IE互換性対応
...
- **AutoComplete**: テキスト入力と連動した候補選択型 UI。TypeScript型定義を改善し、日本語UXを向上。
  - **主な引数**:
    - `name`: コンポーネント識別名（必須）
    - `id`: HTML DOM識別子（オプション）
    - `options`: 候補データ配列（`label`, `value`を含むOption型の配列）
    - `defaultValue`: 初期選択される値（value文字列）
    - `disabled`: 無効化状態設定（デフォルト: false）
    - `helperText`: 補助テキストの表示
    - `error`: 補助テキストとコンポーネントのエラー表示切り替え
    - `freeSolo`: フリーワード入力の許可（デフォルト: false）
    - `onBlur`: フォーカスアウト時のイベントハンドラ
    - `onChange`: 選択変更時のハンドラ（Option | null を返却）
    - `onInputChange`: 入力値変更時のハンドラ（入力文字列を返却）
    - `customStyle`: 外観カスタマイズ用のスタイル指定（MUI sx形式）
  - **機能**: 入力と連動した候補フィルタリング、フリーソロモード対応、補助/エラー表示、カスタムスタイル、日本語メッセージ対応（noOptionsText）
